    ...
    <div v-if="!editMode">
      ...
      <form v-if="editMode" class="mt-3 mb-8" @submit.prevent="handleSubmit">
        ...
        <div v-if="post">
          ...
          <input class="textarea-input" type="textarea" required v-model="post.description">
        </div>
      ...
      <button class="delete-button" v-if="user" @click="handleDelete">Delete Post</button>
      <button class="edit-button" v-if="user && !editMode" @click="handleForm">Edit</button>

      <p class="badge-edit" v-if="changedPost">Post got edited</p></div>
...
export default {
  ...
  setup(props) {
    ...
    return { error, post, handleDelete, handleForm, handleSubmit, editMode, user, changedPost }
  }
}
</script>
